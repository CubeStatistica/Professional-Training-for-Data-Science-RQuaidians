...
df_Course <- df_Course %>% mutate(Language_Interest = case_when(Lang_of_Interest == 'Python' ~'Python',
                                                                ...
                                                                Lang_of_Interest == 'pythob' ~'python',
...
```{r}
ggplot(data = data_Lang, aes(x= n, y= Language_Interest))+
  geom_bar(stat= "identity",  fill = "#21B6A8")+
  geom_text(aes(label = n), vjust = -0.2)+
  ggtitle("Count of Language of Interest.")
 
```

## Word cloud of Primary Reason For Taking this Course

```{r}
Cleaned_data <- removeWords(df_Course$`Primary Reason for taking this course?`, stopwords("english"))
wordcloud(Cleaned_data
        , scale=c(5,0.5)    
        , max.words=100
...
## Table of Prior Formal Education (course teaching or using R, university assignments, coding projects, course work, etc

```{r}
data_R_Education <- df_Course %>% group_by(`Prior Formal Education (course teaching or using R, university assignments, coding projects, course work, etc.) in R?`)%>% count()%>% arrange(desc(n))
perc(data_R_Education)
``` 


### Bar chart of Prior Formal Education (course teaching or using R, university assignments, coding projects, course work, etc

```{r}
ggplot(data = data_R_Education, aes(x= n, y= `Prior Formal Education (course teaching or using R, university assignments, coding projects, course work, etc.) in R?`))+
  geom_bar(stat= "identity",  fill = "#21B6A8")+
  geom_text(aes(label = n), vjust = -0.2)+
  ggtitle("Count of R Prioir Knowledge.")
